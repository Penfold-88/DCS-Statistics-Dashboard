const serversConfig = window.DCS_SERVERS_CONFIG || {};
const serverCardVisibility = serversConfig.serverCardVisibility || {};
const serverDetailFeatures = serversConfig.detailFeatures || {};
const maskExtensionSecrets = Boolean(serversConfig.maskExtensionSecrets);
const publicDateFormat = serversConfig.publicDateFormat || 'd/m/Y';
const i18n = serversConfig.i18n || {};
    ...
    card.innerHTML = `
        <div class="server-detail-header">
            <div>
                ...
                ${serverDetailFeatures.description ? `<p class="server-description">${escapeHtml(server.description || '')}</p>` : ''}
            </div>
            ${serverDetailFeatures.status ? `<span class="${statusClass}">${escapeHtml(status)}</span>` : ''}
        </div>
        ${(serverDetailFeatures.mission || serverDetailFeatures.slots || serverDetailFeatures.restart) ? `
            <div class="detail-metrics">
                ${serverDetailFeatures.mission ? `
                ...
                ` : ''}
                ${serverDetailFeatures.slots ? `<div><span>${escapeHtml(i18n.slots)}</span><strong>${escapeHtml(summary.slotSummary)}</strong></div>` : ''}
                ${serverDetailFeatures.restart ? `<div><span>${escapeHtml(i18n.restart)}</span><strong>${escapeHtml(restart)}</strong></div>` : ''}
            </div>
        ` : ''}
        ${(serverDetailFeatures.weather || serverDetailFeatures.extensions || serverDetailFeatures.activePlayers) ? `
            <div class="detail-split">
                ${serverDetailFeatures.weather ? `
                    <section>
                        <h4>${escapeHtml(i18n.weather)}</h4>
                        <p>${escapeHtml(weather)}</p>
                    </section>
                ` : ''}
                ${serverDetailFeatures.extensions ? `
                    <section>
                        <h4>${escapeHtml(i18n.extensions)}</h4>
                        ${extensions}
                    </section>
                ` : ''}
                ${serverDetailFeatures.activePlayers ? `
                    <section>
